...
- 그리드 시스템은 반응형 디자인을 지원해 웹 페이지를 모바일, 테블릿, 데스크탑 등 다양한 기기에서 적절하게 표시할 수 있도록 돕는다.
- 12개인 이유는 약수가 굉장히 많기 때문이다. 이는 상황별로 칸 수를 조정할 수 있다.
- 이 시스템은 다음과 같은 요소가 있다.  
  - 1. container- column들을 담고 있는 공간. 부트스트랩에 있는 코드다.
  - 2. column - 실제 컨텐츠를 포함하는 부분
  - 3. gutter - 컬럼과 컬럼 사이의 여백 영역  
    - 그리드 시스템에서 컬럼 사이의 여백 영역인데 x축은 패딩, y축은 margin으로 여백을 생성한다.
    - 실제 컬럼 간에 좌우 간격을 변하지 않고 패딩으로 인해 컬럼 안에 컨텐츠의 너비가 변하는 것이다.
    - gx-0 좌우에 패딩을 주지 않겠다.
    - gy는 상하
    - g만 있을 경우, 상하좌우 다 반영된다.
