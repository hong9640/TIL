...
### 서로소 집합
- 교집합이 없다.
- 집합에 속한 하나의 특정 멤버를 통해 각 집합들을 구분하는데 이른 대표자 라고 한다.
- 상호베타 집합을 표현하는 방법:  
  - 연결리스트
  - 트리
- 상호배타 집합 연산:  
...
- 두 정점 사이의 최소비용의 경로 찾기 -> 최단거리
- 신장트리 -> n개의 정점으로 이루어진 양방향 그래프에서 n개의 정점과 n-1개의 간선으로 이루어진 트리
- 최소신장트리 -> 양방향 가중치 그래프에서 신장 트리를 구성하는 간선들의 가중치의 합이 최소인 신장 트리
- 신장트리:  
  - 연결된 그래프 중 모든 정점을 연결, 사이클이 없는 트리 -> n개의 정점과 n-1개로 이루어짐
  - n이 3 이상일 때 사이클이 안생긴다.
  - 하나의 그래프에서 여러 개가 나올 수 있다.
  - prim 알고리즘 -> 특정 정점 기준 작은 것:  
    - 하나의 정점에서 연결된 간선들 중에 하나씩 선택 하면서 mst를 만들어 가는 방식
    - 임의 정점을 하나 선택해서 시작
    - 선택한 정점과 인접하는 정점들 중의 최소비용의 간선이 존재하는 정점을 선택
    - 모든 정점이 선택될 때 까지 위의 과정 2개를 반복
  - kruskal 알고리즘 -> 간선 기준 작은 것:  
...
- 프림: 시간복잡도: O((V + E) logV)
- 보통 정점보다 간선이 더 많다
- -> O(ElogV) E개의 간선을 우선순위 큐에 넣는 시간.
- -> 간선을 모두 고려하면서 우선순위 큐에 넣어야함함
- -> 간선이 많을수록 유리하다.  

- 크루스칼: 시간복잡도: O(ElogE)
- 간선 위주로 정렬하는 시간만 필요
- 간선의 수가 적을 때만 유리하다.
- 프림이 간선의 수가 많으면 느려지기 때문에 크루스칼 먼저 하고 프림 하는 것을 추천

- 다익스트라 알고리즘 -> 최단경로 찾기
- 간선의 가중치가 있는 그래프에서 두 정점사이의 경로들 중에 간선의 가중치의 합이 최소인 경로
- 절대 최장거리 못푼다.
...
- 모든 정점들에 대한 최단 경로  
  - 플로이드-워샬 알고리즘
- 다익스트라는 시작 정점에서 거리가 최소인 정점을 선택해 나가면서 최단 경로를 구하는 방식
- 프림과 비슷하지만 프림의 경우 무조건 작은 것만 고르지만 다익스트라는 누적 거리가 최소인 것을 보는 거다.
- 음수 + 양수가 안되는 이유는 음수가 포함되면 그리디로 접근 자체가 안되기 때문이다. 더 작은 경우가 있을 수 있기 때문이다.예를 들면 바로 가는 루트가 제일 작을 것이 아닌 하나를 거쳐 가는 것이 다 작을 수 있다. 그래서 다익스트라는 음수 + 양수가 안된다.
- 다익스트라 시간복잡도: O((V + E) log V)
**결론**
- 프림은 정점을 기준으로 mst, 작은 가중치부터 고르자. 우선순위 큐 사용됨. 최단거리 구하고
- 크루스칼은 간선을 기준으로 작은 가중치부터 고르자. mst를 생각하자.
- 그리고 최단거리는 다익스트라가 가장 많이 쓰이는데 음수 + 양수가 섞여 있는 문제는 못푼다.
- 양수가 있는 최대거리는 풀 수 있다. 
- 벨만 포드, 플로이드 워셜 이라는 것이 있는데 다익스트라로 해결 못하는 문제들을 풀기 위해 쓰이는데 성능은 안좋고 구현은 쉽다.