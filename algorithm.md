...
### Memoization
- 피보나치를 재귀함수로 구현하는데는 문제점이 있는데 중복 호출이 많다는 것이다.
- 그래서 프로그램 실행 시 이전에 계산한 값을 메모리에 저장해서 다시 계산하지 않도록 하여 실행속도를 빠르게 하는 기술.
- 동적계획법의 핵심이다.
- **메모리에 저장** 이게 핵심이다.
-  if n >= 2 and memo[n] == 0:  
...
### 동적 계획(DP)
- 최적화 문제를 해결하는 알고리즘
- 먼저 입력 크기가 작은 부분을 먼저 해결하고 그 해를 이용하여 더 큰 크기의 부분 묹들을 해결한다.
- 피보나치에 dp를 적용 할 수 있다.
- 문제를 부분 문제로 분할하고 가장 작은 부분 문제부터 해를 구한다.
...
### DFS
- 깊이우선탐색을 뜻한다.
- 깊이 우선 탐색(DFS)은 시작 노드에서 출발해 갈 수 있는 경로를 한 방향으로 끝까지 탐색한 뒤, 
- 더 이상 진행할 수 없으면 직전에 방문했던 노드로 되돌아가 다른 경로를 다시 탐색하는 방식입니다.
- 총 4가지 구현 방식이 있다.  
  - 인접 행렬 + 스택  
  - 인접 리스트 + 스택 
  - 인접 행렬 + 재귀 함수  
  - 인접 리스트 + 다른 input 구조  
- 전에 싸피 문제에서 풀었던 경로 문제가 dp이다.
- dp는 갈 수 있는 곳을 가고 가지 못하는 곳을 만나면 마지막에 만났던 갈림길로 들어와 다른 방향으로 탐색을 계속 반복하여 모든 정점을 방문하는 순회방법.
- 스택에 시작점을 push하고 방문하지 않은 곳이 있다면 탐색 방향을 바꾸기 위해 스택을 pop해서 다시 시작점으로 돌아간다.
- 시작할 때 보통 정점이 작은 쪽으로 먼저 간다.

### 그래프
- 정점(Vertex)과 간선(Edge)으로 구성된다.
- 이 관계를 M:N 관계라고 한다.
- 트리나 복잡한 관계 구조를 해결하기 위해 쓴다.
- 정점: 그래프에서 데이터를 담는 지점
- 간선: 정점 간 연결 관계
- 차수: 정점에 연결된 간선의 수
- 그래프 종류 :  
...
- 그래프 성질:  
  - 인접: 두 정점을 잇는 간선이 존재.
  - 경로: 간선을 연속적으로 따라갈 수 있는 경로
  - 사이클: 시작 정점으로 다시 돌아오는 경로
  - 차수: 각 노드에 연결된 간선의 수
- 그래프 표현
- 인접 행렬:  
  ...
  - 그래서 1번 간선에 2, 3, 정점과 연결되어 있다 그렇게 이해하면 된다.

- 그래프 탐색:  
  - DFS, BFS이렇게 2개가 있다.
  - DFS: **한쪽 경로로 깊이** 내려가다가 갈 곳이 없으면 뒤로 돌아오는 방식  
  - 주로 **스택(Stack)** 자료구조나 **재귀 함수**를 활용  
  - 예: 웹 크롤링, 백트래킹 문제 등에서 사용  
  - BFS: **시작 정점으로부터 가까운(인접) 노드**들을 먼저 방문, 그 다음 거리를 차례대로 탐색  
  - **큐(Queue)** 자료구조로 구현 
...
- 중위표기식의 푸위표기식 변환 방법:  
  ...
  - 괄호를 제거한다.
  - ex) A*B-C/D -> ((A*B) - (C/D)) ->  ((A B)* (C/D))- -> AB*CD/-
- 중위표기법에서 후위표기법으로의 변환 알고리즘:  
  - 입력받은 중위 표기식에서 토큰을 읽는다.
  - 토큰이 피연산자이면 토큰을 출력한다.
  - 토큰이 연산자일때, 이 토큰이 스택의 top에 저장되어 있는 연산자보다 우선순위가 높으면 스택에 push한다.
  - 그렇지 않다면 스택 top의 연산자의 우선순위가 토큰의 우선순위보다 작을 때까지 스택에서 pop한 후 토큰의 연산자를 push 한다. 민액 top에 연산자가 없으면 push 한다.
  - 토큰이 오른쪽 괄호이면 스택 top에 왼쪽 괄호가 올 때까지 스택에 pop연산을 수행하고 pop한 연산자를 출력한다. 왼쪽 괄호를 만나면 pop만 하고 풀력하지는 않는다.
  - 중위 표기식에 더 읽을 것이 없다면 중지하고 더 읽을 것이 있다면 1부터 다시 반복한다
  - 스택에 남아 있는 연산자를 모두 pop하여 출력한다.
  - 우선순위는 토큰에서 ), */, +-, ( 순이다. 숫자는 -, 2, 1, 0 순이다.
- 계산기2
- 후위표기법의 수식을 스택을 이용하여 계산:  
  - 피연산자를 만나면 스택에 push한다.
  - 연산자를 만나면 필요한 만큼의 피연산자를 스택에서 pop하여 연산하고, 연산 결과를 다시 스택에 push 한다.
  - 수식이 끝나면, 마지막으로 스택을 pop하여 출력한다.
### 백트래킹
- 해를 찾는 동중에 막히면 되돌아가서 다시 해를 찾아가는 기법
...
- 결정문제: 문제의 조건을 만족하는 해가 존재하는지의 여부를 'yes' 또는 'no'가 답하는 문제  
  ...
  - Map coloring
  - 부분 집합의 합 문제 등
- 백트래킹과 깊이우선탐색은 차이가 있다.
- 백트래킹은 불필요한 경로를 조기에 차단
- 깊이우선탐색을 가하기에는 경우의 수가 너무나 많음. 즉 N!가지의 경우의 수를 가진 문제에 대한 깊이우선탐색을 가하면 당연히 처리 불가능한 문제
- 백트래킹 알고리즘을 적용하면 일반적으로 경우의 수가 줄어들지만 이 역시 최악의 경우에는 여전히 지수함수 시간을 요하므로 처리 불가능
- 백트래킹은 모든 후보를 검사하는 것은 아니다.
- 어떤 노드의ㅣ유망성을 점검한 후에 유망하지 않다고 결정되면 그 노드의 부모로 되돌아가 다음 자식 노드로 감
...
- 큐의 뒤에서는 삽입만, 큐의 압펭서는 삭제만 이루어진다.
- 선입선출이다.
- 삽입을 enQueue, 삭제를 deQueue라고 한다. 머리는 Front, 꼬리는 Rear이라고 한다.
- 큐 연산 종류:  
  ...
  -  peek: 큐의 가장 앞(front) 데이터를 확인(제거 X)
  -  get_size: 큐에 들어 있는 요소 개수 확인
- 진행 방식은 다음과 같다.  
  - [][][] 인 리스트가 있다 가정하자.
  - 그럼 공백 큐를 생성할 때, front, rear는 모두 -1이다.
  - 그리고 원소 A를 삽입하면 front는 그대로 -1이고 rear는 0으로 넘어간다.
  - [A][][]
  - B를 삽입하면 front는 그대로 -1, rear는 1이 된다.
  - [A][B][], 이때 1번 인덱스가 rear
  - 원소를 삭제하면 front는 0으로 이동한다.
  - [][B][] 이때 0번 인덱스가front, 1번 인덱스는 rear이다.
- 선형큐는 1차원 배열을 이용한 큐로 큐의 크기와 배열의 크기가 같다.
...
- rear = n-1인 상태 즉, 포화상태로 인식하여 더이상 삽입을 수행하지 않게 됨.
- 이를 해결하기 위해 원형 큐의 구조를 사용한다.
- 원형 큐:  
  - front = rear = 0
  - front 변수는 공백상태와 포화 상태 구분을 쉽게 하기 위해 front가 있는 자리는 사용하지 않고 항상 빈자리로 둔다.
  - 삽입 위치: rear = (rear + 1) mod n 
  - 삭제 위치: front = (front + 1) mod n 
  - 원형큐는 선형큐와 다르게 rear가 끝에서 처음으로 순환하여 새 값을 추가할 수 있다.
  - 이를 위해 mod 연산자를 활용한다.
- 연결 큐:  
...
- deque 객체: 양쪽 끝에서 빠르게 추가와 삭제를 할 수 있는 리스트뷰 컨테이너
- 연산은 apeend, popleft를 쓴다.
- 우선순위 큐  
  - 우선순위를 가진 항목들을 저장하는 큐
  - fifo순서가 아니라 운전순위가 높은 순서대로 먼저 나가게 된다.  
  - 주로 시뮬레이션 시스템, 네트워크 트래픽 제어, 운영체제의 테스크 스케줄링  
  - 배열을 이용하여 자료를 저장하고 가장 앞에 최고 우선순위의 원소가 위치락 됨.
  - 하지만 원소의 재배치가 발생한다. 그리고 이에 소요되는 시간이나 메모리 낭비가 크다.
- 버퍼: 데이터를 한 곳에서 다른 한곳으로 전송하는 동안 일시적으로 그 데이터를 보관하는 메모리의 영역
- 버퍼링: 버퍼를 활용하는 방식 또는 버퍼를 채우는 동작을 의미한다.
- 버퍼는 일반적으로 입출력 및 네트워크와 관계된 기능에서 이용된다.
- 순서대로 입력/출력/전달되어야 하므로 FIFO방식의 자료구조인 큐가 활용된다. 
- **참고**
- 선형 큐: `rear`가 배열의 끝에 도달하면 더 이상 삽입하지 못하는 문제(공간 재활용 불가)
- 원형 큐: `front`와 `rear`를 **모듈로 연산**으로 관리해, 공간을 재활용하고 배열을 원형 구조처럼 사용
- 큐가 공백인지, 가득 찼는지는 front와 rear의 상대적 위치로 판별
### BFS
- 너비 우선 탐색
- 인접한 정점들을 모두 차례로 방문한 후에, 방문했던 정점들을 시작점으로 하여 다시 인접한 정점들을 차례로 방문하는 방식
- 선입선출인 큐를 사용한다. 